...
in
{
  ...
  system.activationScripts.rofiConfig = lib.stringAfter [ "users" ] ''
    mkdir -p /home/${username}/.config/rofi
    cat > /home/${username}/.config/rofi/config.rasi << EOL
configuration {
...
EOL
    ...
    cat > /home/${username}/.config/rofi/catppuccin-mocha.rasi << EOL
* {
    bg-col:  #1e1e2e;
    bg-col-light: #1e1e2e;
    border-col: #1e1e2e;
