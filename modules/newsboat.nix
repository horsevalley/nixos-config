...
let
  newsboatConfig = ''
    ...
    color listnormal_unread   #a6e3a1   default
    color listfocus           #f5e0dc   #1e1e2e
    color listfocus_unread    #f5e0dc   #1e1e2e bold
    color info                #cba6f7   #1e1e2e
    color article             #cdd6f4   default

    # Highlights
    highlight article "^(Feed|Link):.*$" #f5c2e7 default bold
    highlight article "^(Title|Date|Author):.*$" #f5c2e7 default bold
    highlight article "https?://[^ ]+" #f9e2af default underline
    highlight article "\\[[0-9]+\\]" #f5c2e7 default bold
    highlight article "\\[image\\ [0-9]+\\]" #f5c2e7 default bold
    highlight feedlist "^─.*$" #f5c2e7 default bold
